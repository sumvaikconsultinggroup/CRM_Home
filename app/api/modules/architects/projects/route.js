...
export async function GET(request) {
  try {
    ...
    console.error('Architects Projects GET Error:', error)
...
export async function POST(request) {
  try {
    ...
    console.error('Architects Projects POST Error:', error)
    if (error.message === 'Unauthorized' || error.message.includes('Forbidden')) {
      return errorResponse(error.message, 401)
    }
...
export async function PUT(request) {
  try {
    ...
    console.error('Architects Projects PUT Error:', error)
    if (error.message === 'Unauthorized' || error.message.includes('Forbidden')) {
      return errorResponse(error.message, 401)
    }
...
export async function DELETE(request) {
  try {
    ...
    console.error('Architects Projects DELETE Error:', error)
    if (error.message === 'Unauthorized' || error.message.includes('Forbidden')) {
      return errorResponse(error.message, 401)
    }
