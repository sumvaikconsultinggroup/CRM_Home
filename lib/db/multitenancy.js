...
export function isValidClientId(clientId) {
  ...
  // Support both new format (CL-XXXXXX) and old format (client-XXX) during migration
  const newPattern = /^CL-\d{6}$/
  const oldPattern = /^client-\d+$/
  // Also support direct database names from migration
  const dbNamePattern = /^client_cl_\d{6}$/
  return newPattern.test(clientId) || oldPattern.test(clientId) || dbNamePattern.test(clientId)
}
