 ...
 * Maps CRM roles to module-specific roles for permission checks
 */
export async function getUserFlooringRole(user) {
  try {
    ...
    // First check if user has a module-specific role assignment
    const assignment = await userRolesCollection.findOne({ userId: user.id })
    if (assignment) {
      return assignment.roleId
    }
    
    // Map CRM roles to module roles
    // Full access roles
    if (['super_admin', 'admin', 'client_admin', 'owner'].includes(user.role)) {
      return 'super_admin'  // Full access to everything
    }
    
    // Management roles
    if (['manager', 'project_manager', 'operations_manager', 'sales_manager'].includes(user.role)) {
      return 'manager'  // Can manage inventory, orders, approve quotes, view reports
    }
    
    // Finance roles
    if (['finance_manager', 'accountant'].includes(user.role)) {
      return 'manager'  // Finance roles get manager-level access for inventory/finance
    }
    
    // Sales roles
    if (['sales_rep', 'sales', 'salesperson', 'executive'].includes(user.role)) {
      return 'sales'  // Can create quotes, manage customers, view inventory
    }
    
    // Warehouse/Operations roles
    if (['warehouse', 'installer', 'surveyor'].includes(user.role)) {
      return 'warehouse'  // Can manage inventory, challans, GRN
    }
