...
async function testQualityControlAPI() {
  ...
  // Test 2: POST start inspection
  await runTest('POST /qc - Start inspection', async () => {
    ...
    const data = await res.json()
    assert(res.ok, `Expected 200, got ${res.status}: ${data.error || ''}`)
    assert(data.qc || data.qcNumber, 'Expected QC record')
    testQCId = data.qc?.id || data.id
  })

  // Test 3: GET single QC record
  await runTest('GET /qc?id=X - Get QC detail', async () => {
    if (!testQCId) throw new Error('No test QC ID')
    const res = await fetch(`${API_BASE}/qc?id=${testQCId}`, { headers: getHeaders() })
    const data = await res.json()
    assert(res.ok, `Expected 200, got ${res.status}`)
    assert(data.id || data.qcNumber, 'Expected QC data')
  })

  // Test 4: POST record check
  await runTest('POST /qc - Record checklist item', async () => {
    if (!testQCId) throw new Error('No test QC ID')
    const res = await fetch(`${API_BASE}/qc`, {
      method: 'POST',
      headers: getHeaders(),
  ...
  // Test 5: POST add defect
  await runTest('POST /qc - Add defect', async () => {
    if (!testQCId) throw new Error('No test QC ID')
    const res = await fetch(`${API_BASE}/qc`, {
      method: 'POST',
      headers: getHeaders(),
      body: JSON.stringify({
        action: 'add_defect',
        qcId: testQCId,
        description: 'Minor scratch on frame',
        category: 'finish',
        severity: 'minor'
      })
    })
  ...
  // Test 6: POST complete inspection
  await runTest('POST /qc - Complete inspection', async () => {
    if (!testQCId) throw new Error('No test QC ID')
    const res = await fetch(`${API_BASE}/qc`, {
      method: 'POST',
      headers: getHeaders(),
